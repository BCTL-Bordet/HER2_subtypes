...
calc_HER2_groups <- function(d,  # d = gene expression data (rows are gene symbols, columns are samples)
...
{ #if (is(d, "Matrix")) { colMeans=Matrix::colMeans; colSums=Matrix::colSums; }
  ...
  { ret = sapply(sig, function(i) calcSig(d, i));
